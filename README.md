...
Install a published package into the Web profile:

```sh
dsh plugin --profile web add dsh-codex
```

To install directly from GitHub, pin a commit:
...
Then repeat the install. Start or restart the Web process and select **Codex default** or a model reported by the local Codex CLI.
...
`pnpm pack` produces the registry artifact. Test an artifact in a new DSH profile before release:
...
For the GitHub and npm release setup, see [PUBLISHING.md](PUBLISHING.md).
